...
/// <summary>
/// Sample code to retrieve template metrics using the Fluent v2 RESTful Engine Client in Java
/// </summary>
public class RetrieveMetrics {
    ...
    public static void main(String[] args) throws Exception {
         ...
         * --------------------------------------------------------------------------------------------
         */
        System.out.println("Requesting template metrics...");

        // Post our template metrics request to the RESTful Engine
        Metrics metrics = client.postMetrics(template);

        // Wait until the metrics request is complete before proceeding
        while (true) {
            Thread.sleep(100);
            int status = client.getMetricsStatus(metrics.getGuid());
            if (status == 302) {
                ...
                System.out.println("Error retrieving template metrics. Status code: " + status);
                break;
            }
        }
         ...
         * --------------------------------------------------------------------------------------------
         */
        System.out.println("Retrieving the template metrics...");
        metrics = client.getMetrics(metrics.getGuid());

        // Write out the template metrics to a file
        File outputFile = new File("files\\Metrics.json");
        try {
            ObjectMapper mapper = new ObjectMapper();
            mapper.writeValue(outputFile, metrics);
        } catch (Exception e) {
